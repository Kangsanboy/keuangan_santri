...
const Index = () => {
  ...
  // 1. Cek Role User saat Login (Apakah Admin, Viewer, atau Parent)
  useEffect(() => {
    const checkUserRole = async () => {
        ...
        if (data) {
            ...
            // Jika dia Parent DAN punya link ke santri -> Kunci Tampilan
            if (data.role === 'parent' && data.linked_santri_id) {
                setDetailSantriId(data.linked_santri_id); // Otomatis buka detail anak
                setSidebarOpen(false); // Tutup sidebar
                setActiveMenu("santri"); // Paksa menu ke santri
            }
        }
    };
    checkUserRole();
  ...
  // 2. Fetch Data Keuangan (Dashboard)
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans relative">
      ...
      {/* SIDEBAR (Hanya muncul jika BUKAN Orang Tua) */}
      {!isParent && (
        <aside
          className={`
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative w-full">
            ...
            {!isParent ? (
              ...
              >
               ...
               <div className="flex items-center gap-2 text-green-800 font-bold">
                   <GraduationCap className="h-6 w-6" /> PPS AL-JAWAHIR (Wali Santri)
               </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 max-w-[60%]">
                <div className="text-right hidden sm:block truncate">
                    ...
                    <p className="text-xs text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded-full inline-block">
                        {isParent ? "Orang Tua" : (isAdmin ? "Admin" : "Viewer")}
                    </p>
                </div>
        ...
        <main className="flex-1 overflow-y-auto p-3 md:p-8 bg-gray-50/50 w-full">
          <div className="max-w-7xl mx-auto space-y-6 pb-20">
            ...
            {/* 🔥 JIKA PARENT LOGIN -> LANGSUNG TAMPIL DETAIL, MENU LAIN HILANG */}
            {isParent ? (
                detailSantriId ? (
                    <SantriDetail 
                        santriId={detailSantriId} 
                        onBack={() => {}} // Tombol back dimatikan
                    />
                ) : (
                    <div className="text-center py-20 text-gray-500">
                        <p className="font-bold mb-2">Akun belum terhubung</p>
                        <p className="text-sm">Silakan hubungi Admin untuk menghubungkan akun Anda dengan data santri.</p>
                    </div>
                )
            ) : (
                /* 🔥 JIKA ADMIN/VIEWER -> TAMPIL NORMAL */
                <>
                    {/* --- DASHBOARD --- */}
                    {activeMenu === "dashboard" && (

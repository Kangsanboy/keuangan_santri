        ...
        <header className="bg-white h-16 flex items-center justify-between px-4 md:px-6 shadow-sm z-10 border-b flex-shrink-0">
          ...
          <div className="flex items-center gap-3 max-w-[60%]">
                <div className="text-right hidden sm:block truncate">
                    ...
                    <p className="text-xs text-green-600 font-medium bg-green-50 px-2 py-0.5 rounded-full inline-block">{isParent ? "Orang Tua" : (isAdmin ? "Admin" : "Viewer")}</p>
                </div>
                    ...
                    {activeMenu === "dashboard" && (
                       <div className="space-y-6 animate-in fade-in zoom-in duration-300">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              {[{ title: "Total Saldo", value: totalMasuk - totalKeluar, color: "text-green-600" }, { title: "Masuk 7 Hari", value: masuk7Hari, color: "text-green-600" }, { title: "Keluar 7 Hari", value: keluar7Hari, color: "text-red-600" }, { title: "Keluar Hari Ini", value: keluarHariIni, color: "text-orange-600" }].map((item, idx) => (<div key={idx} className="border border-green-500 rounded-xl bg-white shadow-sm p-3 text-center flex flex-col justify-center min-h-[100px]"><h4 className="text-[10px] md:text-xs font-bold text-gray-700 mb-1 uppercase tracking-wide">{item.title}</h4><p className={`text-sm md:text-xl font-bold ${item.color} break-words`}>Rp {item.value.toLocaleString("id-ID")}</p></div>))}
                          </div>
                          <div className="border border-green-500 rounded-xl bg-white shadow-sm p-4 overflow-x-auto"><h3 className="text-center font-bold text-gray-800 mb-4 text-sm md:text-lg">Grafik Mingguan</h3><div className="min-w-[300px]"><FinanceChart pemasukan={masuk7Hari} pengeluaran={keluar7Hari} /></div></div>
                       </div>
                    )}
                    {activeMenu === "keuangan" && isAdmin && (
                        <div className="space-y-6 animate-in fade-in zoom-in duration-300">
                           ...
                           <Card className="border-green-200 bg-white shadow-sm overflow-hidden">
                              <CardHeader className="bg-green-50/50 border-b border-green-100 pb-3 p-4"><div className="flex items-center gap-2 text-green-800"><FileSpreadsheet className="w-5 h-5" /><CardTitle className="text-base md:text-lg">Laporan Bulanan</CardTitle></div></CardHeader>
                              <CardContent className="p-4">
                                  <div className="flex flex-col gap-3">
                                      <div className="flex gap-2"><div className="flex-1"><label className="text-xs font-medium text-gray-600">Bulan</label><select value={exportMonth} onChange={(e) => setExportMonth(parseInt(e.target.value))} className="w-full p-2 border border-gray-300 rounded-md text-sm">{monthsList.map((m, idx) => (<option key={idx} value={idx}>{m}</option>))}</select></div><div className="w-24"><label className="text-xs font-medium text-gray-600">Tahun</label><select value={exportYear} onChange={(e) => setExportYear(parseInt(e.target.value))} className="w-full p-2 border border-gray-300 rounded-md text-sm">{yearsList.map((y) => (<option key={y} value={y}>{y}</option>))}</select></div></div>
                                      <Button onClick={exportExcelBulanan} className="bg-green-700 hover:bg-green-800 shadow-md w-full"><FileSpreadsheet className="mr-2 h-4 w-4" />Unduh Excel</Button>
                                  </div>
                              </CardContent>
                           </Card>
                           ...
                           <Card className="border-green-200 bg-white shadow-sm overflow-hidden">
                              ...
                              <CardContent className="p-0">{trxHariIni.length === 0 ? (<div className="p-8 text-center text-gray-500 text-sm">Belum ada transaksi di tanggal ini.</div>) : (<div className="divide-y divide-gray-100">{trxHariIni.map((trx, idx) => (<div key={idx} className="p-4 flex items-center justify-between hover:bg-green-50/30 transition-colors"><div className="flex items-center gap-3">{trx.type === 'income' ? (<ArrowUpCircle className="text-green-600 w-8 h-8 opacity-80" />) : (<ArrowDownCircle className="text-red-500 w-8 h-8 opacity-80" />)}<div className="flex flex-col"><span className="font-bold text-gray-800 text-sm">{trx.santri ? trx.santri.nama_lengkap : "Tanpa Nama"}</span><span className="text-xs text-gray-500 flex gap-2"><span>{trx.santri ? `Kelas ${trx.santri.kelas}` : "-"}</span><span>•</span><span className="italic">{trx.description || "Tanpa Keterangan"}</span></span></div></div><div className={`font-bold text-sm ${trx.type === 'income' ? 'text-green-700' : 'text-red-600'}`}>{trx.type === 'income' ? '+' : '-'} Rp {trx.amount.toLocaleString("id-ID")}</div></div>))}</div>)}</CardContent>
                           </Card>
                        </div>
                    )}
                    {activeMenu === "santri" && (
                      <div className="animate-in fade-in zoom-in duration-300 space-y-4">
                        {detailSantriId ? <SantriDetail santriId={detailSantriId} onBack={() => setDetailSantriId(null)} /> : (<><div className="flex flex-col md:flex-row md:items-center justify-between bg-white p-3 rounded-lg border shadow-sm gap-2"><h2 className="text-base md:text-lg font-bold text-gray-800">{selectedKelasSantri ? `Data Santri Kelas ${selectedKelasSantri}` : "Data Semua Santri"}</h2>{selectedKelasSantri && <Button variant="outline" size="sm" onClick={() => setSelectedKelasSantri(null)} className="w-full md:w-auto">Tampilkan Semua</Button>}</div><SantriManagement key={selectedKelasSantri || 'all'} kelas={selectedKelasSantri ? String(selectedKelasSantri) : null} onSelectSantri={handleSelectSantri} /></>)}
                      </div>
                    )}

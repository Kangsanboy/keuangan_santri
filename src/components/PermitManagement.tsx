...
const PermitManagement = () => {
      ...
      try {
          const { data: pData, error: pErr } = await supabase.from('student_permits').select(`
              *,
              ...
              teacher:penanggung_jawab_id(full_name)
  ...
  const handleTapping = async (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (!santri) {
          ...
          return toast({title: "Akses Ditolak", description: "Kartu / QR tidak terdaftar di sistem.", variant: "destructive"});
      }

      // 2. Cari Tiket Izin Aktif 
      ...
      try {
          ...
          if (activePermit.status === 'Menunggu Keluar') {
              const { error } = await supabase.from('student_permits')
                  .update({ status: 'Sedang Keluar', waktu_keluar_aktual: nowIso })
              ...
              const { error } = await supabase.from('student_permits')
                  .update({ status: newStatus, waktu_kembali_aktual: nowIso })
                  .eq('id', activePermit.id);
              if (error) throw error;
          ...
          toast({title: "Sistem Error", description: err.message, variant: "destructive"});
      }
  };
      ...
      <div className="overflow-x-auto bg-white rounded-xl border border-gray-200 shadow-sm">
          <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-gray-600 uppercase text-xs border-b">
                  <tr><th className="p-3 w-10 text-center">No</th><th className="p-3">Santri</th><th className="p-3">Jadwal (Target)</th><th className="p-3">Aktual Mesin</th><th className="p-3">Alasan / Tujuan</th><th className="p-3 text-center">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                  {data.length === 0 ? <tr><td colSpan={6} className="p-8 text-center text-gray-400 italic">Tidak ada data.</td></tr> : data.map((p, i) => (
                      <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                          <td className="p-3 text-center text-gray-400 font-medium">{i+1}</td>
                          <td className="p-3">
                              <p className="font-bold text-gray-800">{p.santri?.nama_lengkap}</p>
                          ...
                          <td className="p-3 text-xs font-mono">
                              ...
                              <div className="flex items-center gap-1 text-green-600"><LogIn size={12}/> {p.waktu_kembali_aktual ? new Date(p.waktu_kembali_aktual).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'}) : '--:--'}</div>
                          </td>
                          <td className="p-3 text-gray-600 text-xs italic max-w-[200px] truncate" title={p.alasan}>{p.alasan}</td>
                          <td className="p-3 text-center"><Badge variant="outline" className={`text-[10px] font-bold ${getStatusColor(p.status)}`}>{p.status}</Badge></td>
                      </tr>
                  ))}
              </tbody>
          </table>
      ...
      <div className="bg-gradient-to-r from-blue-800 to-indigo-700 rounded-2xl p-6 shadow-lg text-white flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="bg-white/10 p-3 rounded-xl backdrop-blur-sm border border-white/20 w-full md:w-96 flex flex-col items-center">
              ...
              <Input 
                  ...
                  className="bg-white text-gray-900 text-center font-mono font-bold border-2 border-blue-300 focus:border-blue-500 shadow-inner h-12 text-lg rounded-lg"
              />
          </div>
      </div>
      ...
      <Tabs defaultValue="perizinan" value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="perizinan" className="space-y-6">
              ...
              <Card className="border-t-4 border-t-gray-500 shadow-sm">
                  <CardHeader className="bg-gray-50/50 pb-3 border-b"><CardTitle className="text-gray-800 flex items-center gap-2 text-sm"><History className="w-5 h-5"/> Riwayat Perizinan Selesai <Badge className="bg-gray-600">{historyPerizinan.length}</Badge></CardTitle></CardHeader>
          ...
          <TabsContent value="perpulangan" className="space-y-6">
              ...
              <Card className="border-t-4 border-t-gray-500 shadow-sm">
                  <CardHeader className="bg-gray-50/50 pb-3 border-b"><CardTitle className="text-gray-800 flex items-center gap-2 text-sm"><History className="w-5 h-5"/> Riwayat Perpulangan Selesai <Badge className="bg-gray-600">{historyPerpulangan.length}</Badge></CardTitle></CardHeader>
